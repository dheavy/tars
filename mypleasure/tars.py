...
import os, sys, urlparse, datetime, requests, bs4, datetime, hashlib
from ConfigParser import ConfigParser
from pymongo import MongoClient
from pymongo.errors import ConnectionFailure
from mypleasure.scrapers import factory
...
class Tars:
  ...
  TARS attempts to equip itself with the proper probe and if successful,
  will fetch the video.

  Scrapers are a system of classes ("probes") generated by a factory,
  ...
  def __init__(self, debug=False):
    ...
    try:
      self.mongo['client'] = MongoClient(conn_string)
    except ConnectionFailure:
      print('TARS could not connect to the MongoDB videostore. Aborting...')
      sys.exit()
  ...
  def send_on_mission(self, args, forceUrl=None):
    ...
    Returns:
      ...
      or None if the probe does not exist for this video.
  ...
  def scrape(self, url, hash=None):
    ...
    # From given URL, define probe.
    self.url = url
    self.probe = self.__set_probe(self.url)

    # If not probe is to be found, change status to notfound and stop.
    # Otherwise fetch all needed data by invoking probe's method.
    if self.probe is not None:
      ...
      # was passed in command line (i.e. we're trying out probes).
      if hash:
        self.__store(video)
      else:
        print(video)
      ...
      if hash:
        ...
        print('Scraper probe not found...')


  def __set_probe(self, url):
    """Attempt determining the probe based on the given URL.

    Args:
      url: The URL to scrape.

    Returns:
      A probe probe class if a matching one is found, otherwise None.
    ...
    # Invoke factory method to instantiate a probe based on URL.
    probe = factory.create(netloc, url)
    return probe
  ...
  def __get_title(self):
    """Get 'title' by invoking related method from current probe."""
    return self.probe.get_title()


  def __get_poster(self):
    """Get 'poster' by invoking related method from current probe."""
    return self.probe.get_poster()


  def __get_method(self):
    """Get 'method' by invoking related method from current probe."""
    return self.probe.get_method()


  def __get_original_url(self):
    """Get 'original_url' by invoking related method from current probe."""
    return self.probe.get_original_url()


  def __get_embed_url(self):
    """Get 'embed_url' by invoking related method from current probe."""
    return self.probe.get_embed_url()


  def __get_duration(self):
    """Get 'duration' by invoking related method from current probe."""
    return self.probe.get_duration()


  def __is_nsfw(self):
    """Get NSFW status from the current probe"""
    return self.probe.nsfw
